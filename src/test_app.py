...
def test_api_predict():
    ...
    with TestClient(app) as client:
        ...
        assert data["category"] == "Livraison" # Devrait classifier en livraison
        assert "sentiment_confidence" in data
        assert "category_confidence" in data
        assert "latency_ms" in data

def test_api_feedback():
    """Vérifie que l'enregistrement de feedback fonctionne."""
    with TestClient(app) as client:
        payload = {
        ...
        assert response.status_code == 200
        assert response.json() == {"status": "success", "message": "Feedback enregistré pour le réapprentissage."}
